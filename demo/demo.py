...
quality = int(sys.argv[3]) # quality to use in the JPEG compression, from 0 to 100
redundancyFlag = int(sys.argv[4]) # flag to use the redundancy method
showDebugFlag = int(sys.argv[5]) # flag to print additional debug data
...
# vectors containing RMSD values and loss rates
rmsds = np.zeros(8, np.float32)
percentLost = np.zeros(8, np.float32)
...
for bit in range(0, 8):
	...
	cv2.imwrite("./out/png/" + imgFileName[5:len(imgFileName) - 4] + "bit" + str(bit) + ".png", stegoImg, (cv2.IMWRITE_PNG_COMPRESSION, 0)) # saves a PNG file with no compression
	cv2.imwrite("./out/jpg/" + imgFileName[5:len(imgFileName) - 4] + "bit" + str(bit) + ".jpg", stegoImg, (cv2.IMWRITE_JPEG_QUALITY, quality)) # saves a JPEG file with determined quality

	# loads the PNG and JPEG image
	resPNG = cv2.imread("./out/png/" + imgFileName[5:len(imgFileName) - 4] + "bit" + str(bit) + ".png")
	resJPG = cv2.imread("./out/jpg/" + imgFileName[5:len(imgFileName) - 4] + "bit" + str(bit) + ".jpg")
	...
	# stores the RMSD and loss rate values
	rmsds[bit] = RMSD(img, resJPG)
	percentLost[bit] = lossRate
	print()

# for some weird reason, it shows another window for a figure1, which does not exist
# simply close that window and analyze the real plot result
fig = plt.figure()
fig, axes = plt.subplots (nrows = 2)
axes[0].plot(percentLost)
axes[0].set_xlabel("Modified bit")
axes[0].set_ylabel("Loss rate")
axes[0].set_ylim([0, 100])
axes[1].plot(rmsds)
axes[1].set_xlabel("Modified bit")
